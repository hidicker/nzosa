...
What it does not do: bright-line sales, mixed-use assets, the main home exemption, depreciation of rental chattels outside the asset register, and income years whose tax rates it does not hold — for those it says so rather than working tax out on a guess. An agent's deductions from rent never reach a bank account, so rent received through a property manager needs grossing up with a manual journal from the agent's annual statement.
