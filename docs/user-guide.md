...
- **Property manager statements**: if a property manager collects your rent, your bank only ever sees what is left after their fees and repairs. Make a property manager account for the property (a current asset, one per manager), code the manager's payments to you to it, and enter their year-end statement: the rent they collected, what they paid out of it, what they paid you, and what they were holding at the start and end. It must add up before it can be saved. It then posts the full rent, the fees and the repairs, and checks that the account holds exactly what the manager says they hold — a payment made on the last day and banked a few days later is recognised as just that. If you changed manager part way through the year, enter each one's statement against their own account.
...
- The **fixed asset** schedule with depreciation. Keep the register on the **Fixed assets** page: add, change, dispose of or remove an asset with a form, and see each asset's depreciation and book value for the year. To enter many at once, download the template, fill it in on a sheet and load it — the same file **Export register** writes, and the same shape as a Xero fixed asset export. Coding a purchase to a fixed asset account on Reconcile offers to add it to the register, filled in from the purchase. When an asset is sold, what it fetched is read from your previous system's disposal journal if there is one; otherwise you enter it when you dispose of it.
