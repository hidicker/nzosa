...
1. **No Developer Guarantees or Warranties:** NZOSA is free software distributed under the GNU Affero General Public Licence, version 3 or later. It is provided strictly *"as is"*, without warranty of any kind, express or implied. Neither the developers who wrote it nor anyone distributing it assume any liability or legal responsibility if a figure, tax classification, or return is incorrect. The compiling accountant, tax agent, or taxpayer remains entirely responsible for every figure filed with Inland Revenue.
...
### Rental Properties and the IR3
A ledger can hold a household's affairs: rental properties, personal accounts and a business side by side. Each rental is an entity with a kind (residential or commercial), a GST registration and owners with shares, and its accounts are assigned to it.

* **GST follows the entity.** An account belonging to an entity marked not registered resolves to no GST ahead of any rule, chart tax code or saved answer, so a residential rental's rates or a household's power bill never reach a return. Its figures are reported including GST, because it cannot claim it; a registered commercial rental's are net.
* **Rental schedules** set each property out as a practitioner's rental summary does — income, expenses, net rental income, in whole dollars with the prior year — and then all rentals together as one statement of profit or loss. They are built from the same postings as the profit and loss, narrowed to the entity's accounts.
* **Expense headings** follow the IR3 schedules and are placed by account name: rates (water rates included), insurance, interest, agent's collection fees (property management included), repairs and maintenance, and other, which is itemised so a heading placed wrongly is visible.
* **Owner shares** are taken line by line and rounded to the cent, as a practitioner's schedule does, so each owner's headings add to their totals and the halves add to the whole.
* **Residential ring-fencing** is applied across the owner's residential properties as a portfolio: deductions claimed are what residential income can absorb, and the excess — with any brought forward from last year, entered by hand — is carried forward rather than set against other income. A commercial rental's net rent is box 23.
* **The IR3** is set out with the 2026 form's box numbers. Salary and PAYE, interest and RWT, dividends with RWT and imputation credits, and PIE income are entered by hand from the income summary and certificates; the rental figures come from the books. Tax is worked on whole dollars of taxable income at the rates in force from 31 July 2024; the ACC earner levy inside PAYE (1.67% for 2026) is taken out of the PAYE credit; the independent earner tax credit is $520, abating by 13 cents a dollar above $66,000 to nothing at $70,000, and is assumed available only until someone says otherwise. Residual income tax, the refund or tax to pay once provisional tax paid is entered, and next year's provisional tax on the standard option (residual income tax plus 5%, in whole dollars, in three instalments) follow.

What it does not do: bright-line sales, mixed-use assets, the main home exemption, depreciation of rental chattels outside the asset register, and income years whose tax rates it does not hold — for those it says so rather than working tax out on a guess. **Property manager statements.** An agent's deductions never reach a bank account, and their payouts fall in later months than the rent. The agent is treated as holding the owner's money: payments from the agent are coded to a property manager account (a current asset of the property, one per agent), and each statement is entered as rent and other income collected, fees and invoices paid out, paid to the owner, and the balance held at the start and end. A statement must reconcile — held at start plus collected, less paid out, less paid to the owner, equals held at end — before it posts. It posts as a journal dated the end of its period: income credited, expenses debited, the net to the property manager account. Each statement is then checked against the ledger: the account's balance at the period end should equal the balance the agent reports, and a difference equal to receipts banked within two weeks after the period is identified as timing. For a rental not registered for GST, fees are entered including GST; for a registered one, the GST on fees still needs a manual journal. Money the agent held when the ledger begins is best brought in with a journal at the prior balance date (the rent it came from belongs to that year), rather than an opening balance that would restart every account.
